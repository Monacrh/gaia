...
import { create } from 'zustand'
import { ClimateDataForLocation } from './climateApi'
...
interface AppState {
  ...
  // Climate data
  climateData: ClimateDataForLocation | null
  ...
  // Actions
  setSelectedLocation: (location: { lat: number; lng: number } | null) => void
  setClimateData: (data: ClimateDataForLocation | null) => void
  setLoadingClimate: (loading: boolean) => void
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setLoadingClimate: (loading) => set({ isLoadingClimate: loading }),
  toggleTemperature: () => set((state) => ({ showTemperature: !state.showTemperature })),
  toggleCO2: () => set((state) => ({ showCO2: !state.showCO2 })),
  togglePollution: () => set((state) => ({ showPollution: !state.showPollution })),
